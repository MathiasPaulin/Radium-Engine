...
// default constructor
CDDSImage::CDDSImage() :
    m_format(GL_RGBA), m_components(0), m_type(TextureNone), m_valid(false) {
}

CDDSImage::~CDDSImage() {
}

void CDDSImage::create_textureFlat( GLenum format, unsigned int components, const CTexture &baseImage) {
    assert(format != 0);
    assert(components != 0);
...
void CDDSImage::create_texture3D( GLenum format, unsigned int components, const CTexture &baseImage) {
    assert(format != 0);
    assert(components != 0);
    assert(baseImage.get_depth() > 1);
...
void CDDSImage::create_textureCubemap( GLenum format, unsigned int components, const CTexture &positiveX, const CTexture &negativeX,
...
void CDDSImage::upload_texture1D() const {
    assert(m_valid);
...
void CDDSImage::upload_texture2D(uint32_t imageIndex, gl::GLenum target) const {
    assert(m_valid);
    assert(!m_images.empty());
...
void CDDSImage::upload_texture3D() const {
    assert(m_valid);
    assert(!m_images.empty());
...
void CDDSImage::upload_textureCubemap() const {
    assert(m_valid);
    assert(!m_images.empty());
